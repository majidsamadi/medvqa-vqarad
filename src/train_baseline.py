...
def main():
    ...
    model = CNNBiLSTM(vocab_size=tokenizer.vocab_size, num_answers=len(answer2id), freeze_cnn=True).to(device)
    ...
    (out_dir / "baseline_history.json").write_text(json.dumps(history, indent=2), encoding="utf-8")
